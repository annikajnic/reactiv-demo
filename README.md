...
### Reflection
I noticed a runtime error "Warning: Do not call Hooks inside useEffect(...), useMemo(...)," or with on the `ContextNavigator` component, which I narrowed down to generate from the 'expo-router' plugin. I normally use Ionic over Expo, so I am not as familiar with debugging this issue. Functionally, the app works as expected. I think in the future, I would prefer to use Expo from scratch instead of the expo template to avoid any miscellaneous errors.
